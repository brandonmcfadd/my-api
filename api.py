...
async def get_metra_trips(user: str, output_type: str = "JSON", token: str = Depends(get_current_username)):
    ...
    try:
        ...
        if output_type.upper() == "JSON":
            ...
            with open(json_file, 'r', encoding="utf-8") as fp:
                json_file_loaded = json.load(fp)
            if userInput == "ALL_USERS":
                return JSONResponse(content=jsonable_encoder(json_file_loaded))
            else:
                return JSONResponse(content=jsonable_encoder(json_file_loaded[userInput]))
            ...
            elif userInput in json_file_loaded:
                for item in json_file_loaded[userInput]:
                    trip_cost = f"{json_file_loaded[userInput][item]['Trip Cost']:.2f}"
                    new_line = f"{userInput},{json_file_loaded[userInput][item]['Date']},{json_file_loaded[userInput][item]['Line ID']},{json_file_loaded[userInput][item]['Run Number']},{json_file_loaded[userInput][item]['Origin']},{json_file_loaded[userInput][item]['Origin Station - Zone']},{json_file_loaded[userInput][item]['Origin Station - Mileage']},{json_file_loaded[userInput][item]['Origin Station - Kilometers']},{json_file_loaded[userInput][item]['Destination']},{json_file_loaded[userInput][item]['Destination Station - Zone']},{json_file_loaded[userInput][item]['Destination Station - Mileage']},{json_file_loaded[userInput][item]['Destination Station - Kilometers']},{json_file_loaded[userInput][item]['Track Miles']},{json_file_loaded[userInput][item]['Track Kilometers']},{trip_cost},{json_file_loaded[userInput][item]['Ticket Type']}"
                    output_text = f"{output_text}\n{new_line}"
            else:
                raise HTTPException(
                    status_code=404, detail='User Not Found')
            return Response(content=output_text, media_type="text/csv", headers={
                "Content-Disposition": f"attachment; filename=metra-trips-{userInput}.csv"})
    except Exception as exc:
        raise HTTPException(
